...
*Implements standalone PSO logic per Chapter 3 Figure 6.
...
# This function interacts with stakeholders variables to simulate swarm behavior.
def runPsoAlgorithm(arrItems, arrPackagesInfo, funcEvaluateSolution, dictCancellationFlag, dictProgressTracker,
    ...
    # Heuristic: Service times initialization
    # Extracts service time for each package to influence velocity
    arrServiceTimes = np.array([p.get('service_time', 1) for p in arrPackagesInfo])
    arrServiceTimes[arrServiceTimes == 0] = 1

    # Initialization of Toolbox and Population
    ...
    try:
        for intGen in range(intMaxGenerations):
            ...
            # updates position based on cognitive, social, and heuristic factors
            for objParticle in arrSwarm:
...
def _updateParticle(objParticle, objGbest, fltPhi1, fltPhi2, fltPhi3, arrServiceTimes):
    ...
    # Bias swaps towards arranging items based on service time priority
    arrHeuristicSwaps = []
