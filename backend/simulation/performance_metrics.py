...
# Computes volume utilization (adjusted) and generates unloading events using recursion.
def calculateAllMetrics(arrPackedItems, fltBinVolume, arrAllPackagesInfo, strAlgorithmName, arrInitialFreeAreas=None):
    
    # Equation 1: Base Volume Utilization (Raw Physics).
    ...
    # Initialized to base, then adjusted via fragment analysis.
    ...
    # Retrieve container dimensions from parent object if available.
    arrBinDims = [0, 0, 0]
    ...
    # Calculate complex unloading metrics.
...
# Used for testing if items can fit into specific voids.
...
def _calculateFragmentedVolumeAndUtilization(arrFreeAreas, arrAllPackages, fltVuBase, fltTotalBinVol):
    ...
    # Iterate through free areas to check for fitting capability.
...
def _hasDirectContact(arrBlockerPos, arrBlockerDims, arrTargetPos, arrTargetDims):
    ...
    # Check vertical proximity tolerance
...
def _hasClearExtractionPath(arrItemPos, arrItemDims, dictAllItems):
    ...
    fltTestX = -100  # Virtual extraction point towards door
    arrTestPos = [float(fltTestX), float(fltItemY), float(fltItemZ)]
    
    blnPathClear = True
    for dictOther in dictAllItems.values():
        # Optimization: skip checking items clearly "behind"
...
# Validates actual physical blocking conditions versus just positional coordinate comparison.
def _isTrulyBlocking(strItemId, strTargetId, dictAllItems):
    ...
    # Fast overlap check first (Axis Aligned 2D check)
    ...
    # Heuristic Threshold: Small overlaps do not constitute full block
...
# Simulates physical gravity stability constraints.
def _hasGravitySupport(arrPos, arrDims, dictItemsInBin):
    
    fltItemY = arrPos[1]
    # Items on floor always supported.
    ...
    for dictOther in dictItemsInBin.values():
        if dictOther['pos'][1] + dictOther['dims'][1] > fltItemY + 1e-4:
            continue
        # Check general overlap before calculating detailed area
    ...
    # Gravity stability threshold defined in methodology.
    if fltSupportPct < 80:
...
# This function uses an exhaustive grid search when an item is "relocated" temporarily.
def _findBestPositionExhaustive(arrItemDims, dictItemsInBin, arrBinDims, arrOriginalPos):
    ...
    for arrRotatedDims in _getUniqueOrientations(arrItemDims):
        ...
        # Grid loop through the container
        for intX in range(0, int(arrBinDims[0] - arrRotatedDims[0]) + 1):
            for intZ in range(0, int(arrBinDims[2] - arrRotatedDims[2]) + 1):
                for intY in range(0, int(arrBinDims[1] - arrRotatedDims[1]) + 1):
                    ...
                    # Heuristic Scoring for replacement
...
def _getItemsOnTopRecursive(strItemId, dictAllItems, objVisited=None):
    ...
    # Recursion step: find items on top of the items found.
    arrItemsOnTopChain = []
...
def _getSmartBlockerStack(strTargetId, dictAllItems):
    ...
    for strBlockerId in arrBlockingItems:
        ...
        # Add items above blocker first to ensure physical removal validity.
    ...
    # Sort to remove top-most items first (simulating gravity-unstacking).
...
def _generateUnloadingSequence(arrPackedItems, dictPackagesInfoMap, strAlgorithmName, arrBinDims, arrInitialFreeAreas=None):
    ...
    # Simple cluster addition for the rest of packages.
    ...
    for strTargetId in arrDeliveryQueue:
        ...
        # C. Relocation Return (Place relocations back into valid spots).
        ...
        # D. Gravity Settle (Stabilize packing after removal).
        blnStable = False
        intIters = 0
